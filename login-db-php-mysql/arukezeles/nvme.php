...
if (isset($_SESSION['felhaszmail'])) { //Amennyiben a seisson tartalma nem üres
    ...
    ?>
        ...
        <h2>NVMe - árukészlet kezelése</h2>

        <form action="nvme.php" method="post">    

        <input type="text" name="ID" placeholder="ID">
        <input type="text" name="name" placeholder="Név">
        <input type="text" name="rmbsec" placeholder="Olvasási sebesség /sec">
        <input type="text" name="wmbsec" placeholder="Írási sebesség /sec">
    ...
    if (isset($_POST['gomb1'])) { 
        $name = $_POST["name"];
        $processor = $_POST["rmbsec"];
        $ramtype = $_POST["wmbsec"];
        $rammaxmhz = $_POST["capacitygb"];
        ...
        if($name && $rmbsec && $wmbsec && $capacitygb && $descrip && $piece && $pricehuf && $barcode && $picture != ""){
            $sq = "INSERT INTO nvme (name,rmbsec,wmbsec,capacitygb,descrip,piece,pricehuf,barcode,picture) VALUES ('$name','$rmbsec','$wmbsec','$capacitygb','$descrip','$piece','$pricehuf', '$barcode', '$picture')";
    ...
    if (isset($_POST['gomb2'])) { 
        ...
        if ($id !="") {
        $sq = "DELETE FROM nvme WHERE ID = '$id' ";
    ...
    if (isset($_POST['gomb3'])) { 
        $name = $_POST["name"];
            ...
            $sq ="SELECT * FROM nvme WHERE (name) LIKE ('%".$name."%')";
            $leker = $db_connect->query($sq);
