...
if (isset($_SESSION['felhaszmail'])) { //Amennyiben a seisson tartalma nem üres
    ...
    ?>
        ...
        <h2>Processzor - árukészlet kezelése</h2>

        <form action="processor.php" method="post">    

        <input type="text" name="ID" placeholder="ID">
        <input type="text" name="name" placeholder="Név">
        <input type="text" name="type" placeholder="pl: 1151,4,1200">
        <input type="text" name="cores" placeholder="Magok száma">
        <input type="text" name="ghz" placeholder="Ram Max MHz">
        <input type="text" name="turboghz" placeholder="Turbo GHz">
        <input type="text" name="ramtype" placeholder="Támogatott ramtípus">
        <input type="text" name="threads" placeholder="Szálak száma">
    ...
    if (isset($_POST['gomb1'])) { 
        ...
        if($name && $type && $cores && $ghz && $turboghz && $ramtype && $threads && $descrip && $piece && $pricehuf && $barcode && $picture != ""){
            $sq = "INSERT INTO processor (name,type,cores,ghz,turboghz,ramtype,threads,descrip,piece,pricehuf,barcode,picture) VALUES ('$name','$type','$cores','$ghz','$turboghz','$ramtype','$threads','$descrip','$piece','$pricehuf', '$barcode', '$picture')";
    ...
    if (isset($_POST['gomb2'])) { 
        ...
        if ($id !="") {
        $sq = "DELETE FROM processor WHERE ID = '$id' ";
    ...
    if (isset($_POST['gomb3'])) { 
        $name = $_POST["name"];
            ...
            $sq ="SELECT * FROM processor WHERE (name) LIKE ('%".$name."%')";
            $leker = $db_connect->query($sq);
                    ...
                    <?php
                        ...
                        ?>
                            ...
                            <tr>
                                ...
                                <th style="background-color:white"><br><?php echo($row["cores"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["ghz"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["turboghz"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["ramtype"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["threads"]); ?></th>
