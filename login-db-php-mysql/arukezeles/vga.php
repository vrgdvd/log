...
if (isset($_SESSION['felhaszmail'])) { //Amennyiben a seisson tartalma nem üres
    ...
    ?>
        ...
        <h2>VGA - árukészlet kezelése</h2>

        <form action="vga.php" method="post">    

        <input type="text" name="ID" placeholder="ID">
        <input type="text" name="name" placeholder="Név">
        <input type="text" name="memsizegb" placeholder="Memória mérete/GB">
        <input type="text" name="minpwr" placeholder="Ajánlott tápteljesítmény/W">
        <input type="text" name="memtypegddr" placeholder="Ramtipus (GDDR 3,5)">
        <input type="text" name="memmhz" placeholder="Memória/MHz">
        ...
        <input type="submit" name="gomb3" value="Készlet beolvasása">
    ...
    if (isset($_POST['gomb1'])) { 
        $name = $_POST["name"];
        $memsizegb = $_POST["memsizegb"];
        $minpwr = $_POST["minpwr"];
        $memtypegddr = $_POST["memtypegddr"];
        $memmhz = $_POST["memmhz"];
        ...
        if($name && $memsizegb && $minpwr && $memtypegddr && $memmhz && $descrip && $piece && $pricehuf && $barcode && $picture != ""){
            $sq = "INSERT INTO vga (name,memsizegb,minpwr,memtypegddr,memmhz,descrip,piece,pricehuf,barcode,picture) VALUES ('$name','$memsizegb','$minpwr','$memtypegddr','$memmhz','$descrip','$piece','$pricehuf','$barcode','$picture')";
    ...
    if (isset($_POST['gomb2'])) { 
        ...
        if ($id !="") {
        $sq = "DELETE FROM vga WHERE ID = '$id' ";
    ...
    if (isset($_POST['gomb3'])) { 
        $name = $_POST["name"];
            ...
            $sq ="SELECT * FROM vga WHERE (name) LIKE ('%".$name."%')";
            $leker = $db_connect->query($sq);
            ...
            if (mysqli_num_rows($eredm) > 0){
                ...
                <th>Név:</th>
                <th>Memória /GB</th>
                <th>Ajánlott táp /W:</th>
                <th>Memória típusa GDDR:</th>
                <th>Mhz:</th>
                <th>Leírás:</th>
                <th>Darabszám:</th>
                <th>Ár (Ft):</th>
                    ...
                    <?php
                        ...
                        ?>
                            ...
                            <tr>
                                ...
                                <th style="background-color:white"><br><?php echo($row["memsizegb"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["minpwr"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["memtypegddr"]); ?></th>
                                <th style="background-color:white"><br><?php echo($row["memmhz"]); ?></th>
